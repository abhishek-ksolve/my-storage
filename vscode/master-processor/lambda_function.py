...
def lambda_handler(event, context):
    ...
    if event['resource']=="/execute":
        ...
        input_param = {"dataset1": dataset1,"dataset2": dataset2,"jobName":jobName,"output":output}
        ...
        try:
            ...
            for algo_list in data['algorithms'][0].items():
                ...
                if "Probabilistic record linkage"== algo_list[0]:
                    ...
                    FunctionName = 'arn:aws:lambda:us-east-1:905744249148:function:problasticRecordLinkage',
                ...
                if "Blocking Based Schema Agnostic"== algo_list[0]:
                    ...
                    FunctionName = 'arn:aws:lambda:us-east-1:905744249148:function:blockingBasedSchemaAgnostic',
                    InvocationType = 'RequestResponse',
                ...
                if "Schema Based"== algo_list[0]:
                    ...
                    FunctionName = 'arn:aws:lambda:us-east-1:905744249148:function:SchemaBased',
                    InvocationType = 'RequestResponse',
                ...
                if "Progressive Schema Agnostic"== algo_list[0]:
                    ...
                    FunctionName = 'arn:aws:lambda:us-east-1:905744249148:function:progressiveSchemaAgnostic',
                    InvocationType = 'RequestResponse',
    ...
    if event['resource']=="/output/job/{jobId}":
        ...
        if result[0][0]==jobId and result[0][1]==None:
            print("parent_id")
            cursor = connection.cursor()
            sql_get_status = """SELECT job_result,job_definition_id FROM er.job where parent_job_uuid =%s """
            cursor.execute(sql_get_status,[jobId])
            ...
            print("child")
            sql_get_status = """SELECT parent_job_uuid,job_result,job_definition_id FROM er.job where job_uuid =%s """
            cursor.execute(sql_get_status,[jobId])
            result = cursor.fetchall();
            parent_jobid1=(result[0][2])
            jobId1=result[0][0]
            result=result[0][1]
            sql_get_algoconfig = """SELECT algo_config FROM er.job_definition where id =%s """
            cursor.execute(sql_get_algoconfig,[parent_jobid1])
            job_details = cursor.fetchall();
            parentJobName=(job_details[0][0]['config']['jobName'])
            response={ "jobUuid": jobId1,"jobName":parentJobName,
                                        "subJob": result}
